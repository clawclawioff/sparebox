  ...
  return new Promise((resolve, reject) => {
    execFile(cmd, args, { timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (err, stdout, stderr) => {
      if (err) {
        reject(
          new Error(
            `${cmd} ${args.join(" ")} failed: ${err.message}${stderr ? " — " + stderr.trim() : ""}`
          )
        );
      } else {
 ...
 */
export async function detectRuntime(): Promise<RuntimeName | null> {
 ...
 */
async function rt(): Promise<string> {
 ...
 */
function parseMemValue(s: string): number {
