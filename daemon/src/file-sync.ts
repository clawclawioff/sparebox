...
export interface FileChange {
  agentId: string;
  filename: string; // relative path from workspace root, e.g. "SOUL.md"
  content: string;
  hash: string;
}
...
export function getChangedFiles(
  ...
  const prevHashes = hashStore.get(agentId) ?? new Map<string, string>();
...
const deletedFilesStore: Map<string, string[]> = new Map();
...
export function getFileChanges(
  agentId: string,
  workspaceDir: string
): { changed: FileChange[]; deleted: string[] } {
