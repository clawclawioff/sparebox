 ...
 */
export async function stopProfile(profileName: string): Promise<void> {
  const bin = await findOpenclawBinary();
  if (!bin) {
 ...
 */
export async function isProfileRunning(pid: number | null): Promise<boolean> {
  if (!pid) return false;
 ...
 */
export async function getProfileStatus(profileName: string, pid: number | null): Promise<ProfileAgent["status"]> {
  const bin = await findOpenclawBinary();
 ...
 */
export async function killProfile(pid: number | null): Promise<void> {
  if (!pid) return;
